...
const exceedLimit = async (event) => {
  ...
  let totalDuration = 0;
  const startOfWeek = moment(event.start).startOf('week').add(1, 'day').toDate();
  const endOfWeek = moment(event.start).endOf('week').add(1, 'day').toDate();

  for (const e of groupEvents.filter(e => moment(e.start).isBetween(startOfWeek, endOfWeek, null, '[]'))) {
